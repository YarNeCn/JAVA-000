...
public class CustomHttpClient implements HttpClient {
    ...
    public void excute(String serverUrl, ChannelHandlerContext serverResponseChannelHandlerContext, FullHttpRequest msg) {
        //这里需要调整一下请求的地址
        final HttpGet httpGet = new HttpGet(serverUrl+msg.getUri());
        ...
        httpclient.execute(httpGet, new FutureCallback<HttpResponse>() {
            ...
            public void completed(final HttpResponse endpointResponse) {
                try {
                    handleResponse(msg, serverResponseChannelHandlerContext, endpointResponse);
                } catch (Exception e) {
                    e.printStackTrace();
                } finally {
            ...
            public void failed(final Exception ex) {
                httpGet.abort();
                ex.printStackTrace();
                //请求失败的话将netty的服务端响应回去
                FullHttpResponse response = new DefaultFullHttpResponse(HTTP_1_1, NO_CONTENT);
                serverResponseChannelHandlerContext.write(response).addListener(ChannelFutureListener.CLOSE);
...
//            System.out.println(body.length);
            ...
            response.headers().set("client", "httpClient");
